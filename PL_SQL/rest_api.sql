...
*/
DECLARE
    CURSOR cs (a_id_user customer_subscriptions.id_user%TYPE) IS SELECT id_product FROM customer_subscriptions WHERE id_user = a_id_user;
...
BEGIN
    ...
    FOR i IN cs(:id_user) LOOP
        json_cs := json_object_t();
        json_cs.put('id_product', i.id_product);
